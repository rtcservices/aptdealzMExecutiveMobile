...
namespace aptdealzMExecutiveMobile.API
{
    public class ProfileAPI
    {
        ...
        #region [ PUT ]
        public async Task<Response> SaveProfile(ExecutiveDetails mExecutiveDetails)
        {
            Response mResponse = new Response();
            try
            {
                if (CrossConnectivity.Current.IsConnected)
                {
                    var requestJson = JsonConvert.SerializeObject(mExecutiveDetails);
                    using (var hcf = new HttpClientFactory(token: Common.Token))
                    {
                ...
                else
                {
                    if (await Common.InternetConnection())
                    {
                        await SaveProfile(mExecutiveDetails);
                    }
                }
            }
            catch (Exception ex)
